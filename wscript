...
def configure(cfg):
    ...
    # Generate bit code
    # cfg.env.CXXFLAGS += ['-emit-llvm','-g','-O0']
    # cfg.env.CXXFLAGS += ['-emit-llvm','-O0']
    cfg.env.CXXFLAGS += ['-emit-llvm','-O0','-fsanitize=cfi', '-fno-sanitize-ignorelist', '-flto','-fvisibility=hidden']
    #  cfg.env.CXXFLAGS += ['-include', 'ap_config.h', '-flto','-O0']#cspl added generate bc code, remember to use -O0, other wise floating error


    # Add flags in linking process
    cfg.env.LINKFLAGS += ['-flto']
