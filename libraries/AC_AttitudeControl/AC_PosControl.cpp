...
// Thread-safe map to store variable values
std::unordered_map<void *, int> variableMap;
std::mutex mapMutex;
// Define a small tolerance for floating-point comparison
constexpr float EPSILON = 1e-6;


// Function to record the address and value of a variable
extern "C" void record_dfi(void *variableAddress, float value) {
    std::lock_guard<std::mutex> lock(mapMutex);
    variableMap[variableAddress] = value;
    // std::cout << "[record] Address: " << variableAddress << ", Value: " << value << std::endl;
}

// Function to check the value of a variable against its recorded value
extern "C" void check_dfi(void *variableAddress, float value) {
    std::lock_guard<std::mutex> lock(mapMutex);
    auto it = variableMap.find(variableAddress);
    // std::cout<< "check_dfi" << std::endl;
    if (it != variableMap.end()) {
        int recordedValue = it->second;
        if (std::abs(recordedValue - value) < EPSILON) {
            // std::cout << "[check] Match at Address: " << variableAddress
            //           << ", Value: " << value << std::endl;
        } else {
            // developer can insert assertion here
            // std::cerr << "[check] Mismatch at Address: " << variableAddress
            //           << ", Recorded: " << recordedValue << ", Current: " << value << std::endl;
        }
    } else {
        // std::cerr << "[check] No record found for Address: " << variableAddress << std::endl;
    }
}
...
{
    // std::cout << "[check] Match at Address: ";
