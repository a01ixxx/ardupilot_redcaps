...
typedef std::vector<std::chrono::nanoseconds> MicrosecondsVector;
typedef std::vector<int> IntsVector;
std::map<std::string, MicrosecondsVector> all_task_exe_times;

std::map<std::string, IntsVector> all_task_count_cspl_data_replay;
std::map<std::string, IntsVector> all_task_count_cspl_data_log;
std::map<std::string, IntsVector> all_task_count_cspl_inter_cpt_transfer;
std::map<std::string, IntsVector> all_task_count_cspl_intra_cpt_transfer;
std::map<std::string, IntsVector> all_task_count_cspl_unknown_ind_call;
std::map<std::string, IntsVector> all_task_count_cspl_bug_crash;
std::map<std::string, IntsVector> all_task_count_cspl_intra_ind_call;
std::map<std::string, IntsVector> all_task_count_cspl_inter_ind_call;
std::map<std::string, IntsVector> all_task_count_cspl_data_inter;
std::map<std::string, IntsVector> all_task_count_cspl_data_global;
std::map<std::string, IntsVector> all_task_count_cspl_data_other;
std::map<std::string, IntsVector> all_task_count_cspl_data_intra;


int count_cspl_data_replay=0;
int count_cspl_data_log=0;
int count_cspl_inter_cpt_transfer=0;
int count_cspl_intra_cpt_transfer=0;
int count_cspl_unknown_ind_call=0;
int count_cspl_bug_crash=0;
int count_cspl_intra_ind_call=0;
int count_cspl_inter_ind_call=0;
int count_cspl_data_inter=0;
int count_cspl_data_global=0;
int count_cspl_data_other=0;
int count_cspl_data_intra=0;


void signalHandler(int signum) {
    // std::cout << "Interrupt signal (" << signum << ") received.\n";
    std::cout << "Interrupt signal received, start to logging execution times\n";
    // Perform cleanup if necessary

     std::cout << "Logging finished, exit!\n";
    // Terminate program
    // exit(signum);

    // To log the execution times to files
    // auto -> std::map<std::string, MicrosecondsVector>::iterator
    for (auto it = all_task_exe_times.begin(); it != all_task_exe_times.end(); ++it) {
        std::string task_name = it->first;
        MicrosecondsVector &execution_times = it->second;

        std::cout << "Task: " << task_name << " has " << execution_times.size() << " execution times\n";

        // Logging the execution times
        task_name = "exe_" + task_name;
        std::ofstream tmp_log_file(task_name);
        if (!tmp_log_file.is_open()) {
        // Handle error (e.g., print an error message and return)
            std::cout << "Openning file error \n";
        }
//         for(unsigned long i=0;i<execution_times.size();i++){
//              tmp_log_file << execution_times[i].count() <<" ";

//              tmp_log_file << all_task_count_cspl_data_replay[task_name][i] <<" ";
// tmp_log_file <<all_task_count_cspl_data_log[task_name][i] <<" ";
//  tmp_log_file <<all_task_count_cspl_inter_cpt_transfer[task_name][i] <<" ";
// tmp_log_file <<all_task_count_cspl_intra_cpt_transfer[task_name][i] <<" ";
// tmp_log_file <<all_task_count_cspl_unknown_ind_call[task_name][i] <<" ";
//  tmp_log_file <<all_task_count_cspl_bug_crash[task_name][i] <<" ";
//  tmp_log_file <<all_task_count_cspl_intra_ind_call[task_name][i] <<" ";
//  tmp_log_file <<all_task_count_cspl_inter_ind_call[task_name][i] <<" ";
//  tmp_log_file <<all_task_count_cspl_data_inter[task_name][i] <<" ";
//  tmp_log_file <<all_task_count_cspl_data_global[task_name][i] <<" ";
//  tmp_log_file <<all_task_count_cspl_data_other[task_name][i] <<" ";
// tmp_log_file << all_task_count_cspl_data_intra[task_name][i] <<" ";
//         tmp_log_file<< std::endl;
//         }
        int m_count = 0;
        for (const auto& duration : execution_times) {
            // std::cout << duration.count() << " ";
            tmp_log_file << duration.count() << std::endl;;
...
void AP_Scheduler::run(uint32_t time_available)
{
 ...
 count_cspl_data_replay=0;
 count_cspl_data_log=0;
 count_cspl_inter_cpt_transfer=0;
 count_cspl_intra_cpt_transfer=0;
 count_cspl_unknown_ind_call=0;
 count_cspl_bug_crash=0;
 count_cspl_intra_ind_call=0;
 count_cspl_inter_ind_call=0;
 count_cspl_data_inter=0;
 count_cspl_data_global=0;
 count_cspl_data_other=0;
 count_cspl_data_intra=0;
        ...
        std::string task_name(_tasks[i].name);

        all_task_exe_times[task_name].push_back(duration);

//         all_task_count_cspl_data_replay[task_name].push_back(count_cspl_data_replay);
// all_task_count_cspl_data_log[task_name].push_back(count_cspl_data_log);
//  all_task_count_cspl_inter_cpt_transfer[task_name].push_back(count_cspl_inter_cpt_transfer);
// all_task_count_cspl_intra_cpt_transfer[task_name].push_back(count_cspl_intra_cpt_transfer);
// all_task_count_cspl_unknown_ind_call[task_name].push_back(count_cspl_unknown_ind_call);
//  all_task_count_cspl_bug_crash[task_name].push_back(count_cspl_bug_crash);
//  all_task_count_cspl_intra_ind_call[task_name].push_back(count_cspl_intra_ind_call);
//  all_task_count_cspl_inter_ind_call[task_name].push_back(count_cspl_inter_ind_call);
//  all_task_count_cspl_data_inter[task_name].push_back(count_cspl_data_inter);
//  all_task_count_cspl_data_global[task_name].push_back(count_cspl_data_global);
//  all_task_count_cspl_data_other[task_name].push_back(count_cspl_data_other);
//  all_task_count_cspl_data_intra[task_name].push_back(count_cspl_data_intra);
