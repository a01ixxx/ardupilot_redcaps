...
void Copter::log_gps_data()
{
    // gps.update();
    start_time = std::chrono::steady_clock::now();
    outfile.open("/tmp/gps_data.txt", std::ios::app);

    if (outfile.is_open()) {
        size_t state_size = sizeof(gps.state[0]);
        outfile.write((char*)&gps.state[0], state_size);
        ////////////////////////////////////
        outfile.close();
    } else {
...
void Copter::update_GPS(void)
{
    ...
    if (copter.control_mode == Mode::Number::AUTO) {
        ...
        {
            update_recovery_block();
            return;
        }
#endif
    } 

    if (native_call_count++ >= 500) {
        native_call_count = 0;
        std::cout << "[Gecko] Using the native block" << std::endl;
    }
...
std::vector<AP_GPS::GPS_State> gps_states;

void open_replay_file(void)
{
    std::ifstream replayfile("/tmp/gps_data.txt");
    if (replayfile.is_open()) {
        AP_GPS::GPS_State state;
        while (replayfile.read((char*)&state, sizeof(state))) {
            // Process the GPS state data
            // std::cout << "[After] State location alt: " << state.location.alt << std::endl;
            gps_states.push_back(state);
        }
        replayfile.close();
    } else {
        std::cerr << "Unable to open file" << std::endl;
...
int recovery_block_call_count = 0;

void Copter::update_recovery_block(void)
{

    if (recovery_block_call_count++ >= 500) {
        recovery_block_call_count = 0;
        std::cout << "[Gecko] Using the recovery block" << std::endl;
    }
    ...
    gps.state[0] = gps_states[mission_index];

    gps.state[0].last_gps_time_ms = AP_HAL::millis();
